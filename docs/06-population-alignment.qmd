...
Zeros of $\boldsymbol{\Theta}$ survive the sum **only when every cell
...
replicate yields
$(\hat{\boldsymbol{\mu}}^{\ast},\widehat{\boldsymbol{\Theta}}^{\ast})$
...
$\widehat{\boldsymbol{\Sigma}}_{j}=\widehat{\boldsymbol{\Theta}}_{j}^{-1}$.
...
$\widehat{\boldsymbol{\Theta}}_{j}$, and the copula is not refit. The
...
\boldsymbol{\Sigma}
=
\widehat{\boldsymbol{\Theta}}_{j}^{-1}.
$$ {#eq-copula-r}
...
matched per-cell depth. Re-fitting $\boldsymbol{\Theta}_{j}$ on it is
...
competitor, not a construction of $\boldsymbol{\Theta}_{j}$.
